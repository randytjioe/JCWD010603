...
export default function Record() {
    ...
    const superAdmin = JSON.parse(localStorage.getItem("data")) ? JSON.parse(localStorage.getItem("data")).isSuperAdmin : null;
    ...
    const fetchAllTransByBranch = (e) => {
        if (e) {
            console.log(e);
            axiosInstance.get(`/transaction/allTransactionByBranch/${JSON.parse(localStorage.getItem("data")) ? JSON.parse(localStorage.getItem("data")).BranchId : null}?search=${e}&page=${page}&limit=${limit}&order=${order}`).then((res) => {
                setAllBranchTrans(res.data.result)
                setPage(res.data.page)
                setPages(res.data.totalPage)
                setRows(res.data.totalRows)
            ...
            axiosInstance.get(`/transaction/allTransactionByBranch/${JSON.parse(localStorage.getItem("data")) ? JSON.parse(localStorage.getItem("data")).BranchId : null}?page=${page}&limit=${limit}&order=${order}`).then((res) => {
                setAllBranchTrans(res.data.result)
                setPage(res.data.page)
                setPages(res.data.totalPage)
                setRows(res.data.totalRows)
    ...
    async function cancelOrder(orderId) {
        try {
            await axiosInstance.patch(`/transaction/admincancelorder/${orderId}`).then(() => {
                superAdmin ?
                    fetchAllTrans()
                    :
                    fetchAllTransByBranch()
    ...
    async function sendOrder(orderId) {
        try {
            await axiosInstance.patch(`/transaction/adminsendorder/${orderId}`).then(() => {
                superAdmin ?
                    fetchAllTrans()
                    :
                    fetchAllTransByBranch()
                                                                            ...
                                                                            </Grid>
                                                                            {
                                                                                superAdmin ? null : (
                                                                                    <Flex w='100%' justify='space-between' mt='4'>
                                                                                        <Button
                                                                                            variant='outline' colorScheme='yellow' onClick={() => cancelOrder(val.id)}
                                                                                        >
                                                                                            <b>Cancel</b>
                                                                                        </Button>
                                                                                        <Button
                                                                                            colorScheme='teal' variant='solid' onClick={() => sendOrder(val.id)}
                                                                                        >
                                                                                            Send
                                                                                        </Button>
                                                                                    </Flex>
